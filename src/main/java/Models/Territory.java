...
public class Territory {
    ...
    public static void conquers(Attack attack,String army ,Session sessionstart,Player playerstart){
        ...
        int territory_amount = name_continent.getTerritory().size();
        int counter = 0;
        for (Continent continent : sessionstart.getMap().getContinents()) {
        ...
        if(playerstart.getContinentAmount() == sessionstart.getMap().getContinents().size() && (sessionstart.getType().equals("World domination risk") || sessionstart.getType().equals("RISK_FOR_TWO_PLAYERS"))){
            System.out.println(playerstart.getContinentAmount()+ "  " + sessionstart.getMap().getContinents().size());
            System.out.println("Ganador");
        }else{
            if(attack.getDefender().getCapital()){
                ArrayList<Territory> capitals = new ArrayList();
                ArrayList<Continent> continents = sessionstart.getMap().getContinents();
